...
class GameScene: SuperScene {
    ...
    var controllerPressedHolder:(GameViewController.JoysticName, Float)?
    ...
    func controllerPressed(key:GameViewController.JoysticName?, pressed:Bool, value:Float) {
        if (key?.waitRelease ?? false) && pressed {
            self.controllerPressedHolder = (key!, value)
        } else if (!pressed && (self.controllerPressedHolder?.0 ?? .unrecognized) == key) {
            controllerPressedHolder = nil
        } else if let key = key {
            switch key {
            case .b:
                player?.spawnBullet()
            case .a, .padUp, .rJoysticTop:
                if jumpPressed < 3 {
                    jumpPressed += 1
                    player?.jump()
                }
